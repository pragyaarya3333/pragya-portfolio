...
if (config) {
  ...
  if (
    ...
    const script = document.getElementById('BMC-Widget');

    if (script) {
      script.setAttribute(
        'data-color',
        config.themeConfig[config.themeConfig.defaultTheme].highlight ||
          config.themeConfig[config.themeConfig.defaultTheme].primary ||
          '#fff'
      );
    }
  }
